...
```bash
DOCKER_BUILDKIT=1 docker compose --profile playwright --env-file test.env up Passw-pro
...
```bash
DOCKER_BUILDKIT=1 docker compose --profile playwright --env-file test.env build Passw-pro
```
...
Additionally this `docker-compose` template allows to run locally Passw-pro,
...
Then start the stack (the `profile` is required to run `Passw-pro`) :

```bash
> docker compose --profile passw-pro --env-file .env up
....
...
- `Passw-pro` on http://0.0.0.0:8000 with the default user `test@yopmail.com/test`.
...
```
When running with a local Passw-pro, you can use a front-end build from [dani-garcia/bw_web_builds](https://github.com/dani-garcia/bw_web_builds/releases).

## Rebuilding the Passw-pro

To force rebuilding the Passw-pro image you can run

```bash
docker compose --profile passw-pro --env-file .env build VaultwardenPrebuild Vaultwarden
```

## Configuration

All configuration for `keycloak` / `Passw-pro` / `keycloak_setup.sh` can be found in [.env](.env.template).
...
- All `Passw-pro` configuration can be set (EX: `SMTP_*`)

## Cleanup

Use `docker compose --profile passw-pro down`.
