# Passw-pro Container Building

To build and release new testing and stable releases of Passw-pro we use `docker buildx bake`.<br>
...
docker buildx create --name passw-pro --use --driver-opt network=host
...
CARGO_PROFILE=dev \
SOURCE_COMMIT="$(git rev-parse HEAD)" \
CONTAINER_REGISTRIES="localhost:5000/passw-pro/server" \
...
docker run --rm -it \
  ...
  passw-pro/server:testing-arm64
```
...
podman run --rm -it \
  ...
  -e I_REALLY_WANT_VOLATILE_STORAGE=true \
  -p8080:80 --platform=linux/arm64 \
  localhost/passw-pro/server:testing-arm64
```
...
| CONTAINER_REGISTRIES  | passw-pro/server   | Comma separated value of container registries. Like `ghcr.io/nickcatsshild/passw-pro,docker.io/passw-pro/server` |
| PW_VERSION            | null               | To override the `SOURCE_VERSION` value. This is also used by the `build.rs` code for example                       |
