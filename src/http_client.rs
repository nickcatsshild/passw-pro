...
pub fn get_reqwest_client_builder(enforce_block: bool) -> ClientBuilder {
    ...
    headers.insert(header::USER_AGENT, header::HeaderValue::from_static("Passw-pro"));
...
mod tests {
    ...
    fn ipv4_mapped_in_ipv6_loopback_blocked() {
        // ::ffff:127.0.0.1 â€” v4-mapped form; is_global_hardcoded blocks via ::ffff:0:0/96
    ...
    fn idn_unicode_gets_punycoded() {
        let h = get_valid_host("deadbeafcafÃ©.test").expect("valid");
        match h {
            Host::Domain(d) => assert_eq!(d, "xn--deadbeafcaf-lbb.test"),
            _ => panic!("expected domain"),
        }
    ...
    fn idn_unicode_gets_punycoded_tld() {
        let h = get_valid_host("deadbeaf.cafÃ©").expect("valid");
        match h {
    ...
    fn idn_emoji_gets_punycoded() {
        let h = get_valid_host("xn--t88h.test").expect("valid"); // ðŸ›¡ï¸.test
        match h {
    ...
    fn idn_unicode_to_punycode_roundtrip() {
        let from_unicode = get_valid_host("ðŸ›¡ï¸.test").expect("valid");
