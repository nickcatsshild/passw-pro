...
impl DbPool {
    ...
    pub fn from_config() -> Result<Self, Error> {
        ...
        if log_enabled!(target: "Passw-pro::db::query_logger", log::Level::Warn)
            || log_enabled!(target: "Passw-pro::db::query_logger", log::Level::Info)
            || log_enabled!(target: "Passw-pro::db::query_logger", log::Level::Debug)
        {
...
impl DbConnType {
    pub fn from_url(url: &str) -> Result<Self, Error> {
        ...
        // No recognized scheme â€” assume legacy bare-path SQLite, but the database file must already exist.
