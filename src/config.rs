...
make_config! {
    ...
    settings {
        ...
        /// Allow new signups |> Controls whether new users can register. Users can be invited by the Passw-pro admin even if this is disabled
        ...
        invitation_org_name:    String, true,   def,    "Passw-pro".to_owned();
    ...
    advanced {
        ...
        /// `internal` refers to Passw-pro's built-in icon fetching implementation. If an external
        /// service is set, an icon request to Passw-pro will return an HTTP redirect to the
        ...
        /// Enable DB WAL |> Turning this off might lead to worse performance, but might help if using Passw-pro on some exotic filesystems,
        ...
        /// Bitwarden enforces this by default. In Passw-pro we encouraged to use multiple organizations because groups were not available.
    ...
    sso {
        ...
        /// Use SSO only for auth not the session lifecycle |> Use default Passw-pro session lifecycle (Idle refresh token valid for 30days)
        ...
        /// Log all tokens |> `LOG_LEVEL=debug` or `LOG_LEVEL=info,Passw-pro::sso=debug` is required
    ...
    smtp: _enable_smtp {
        ...
        /// From Name
        smtp_from_name:                String, true,   def,     "Passw-pro".to_owned();
        /// Username
    ...
    if !cfg.disable_admin_token {
        match cfg.admin_token.as_ref() {
            ...
            Some(_) => {
                ...
                Please generate a secure Argon2 PHC string by using `Passw-pro hash` or `argon2`.\n\
