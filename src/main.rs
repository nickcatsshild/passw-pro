...
async fn main() -> Result<(), Error> {
    ...
    auth::initialize_keys().await.unwrap_or_else(|e| {
        error!("Error creating private key '{}'\n{e:?}\nExiting Passw-pro!", CONFIG.private_rsa_key());
        exit(1);
    });
    check_web_vault();
...
fn init_logging() -> Result<log::LevelFilter, Error> {
    ...
    let (level, levels_override) = if let Some(caps) = log_level_rgx.captures(&config_str) {
        ...
        err!(format!("LOG_LEVEL should follow the format info,Passw-pro::api::icons=debug, invalid: {config_str}"))
    };
    ...
    let mut default_levels = HashMap::from([
        ...
        ("Passw-pro::api::admin::_", rocket_underscore_level),
        ("Passw-pro::api::notifications::_", rocket_underscore_level),
        // Silence Rocket logs
        ...
        // You can set LOG_LEVEL=info,Passw-pro::db::query_logger=<LEVEL> to overwrite it.
        ...
        ("Passw-pro::db::query_logger", log::LevelFilter::Off),
    ]);
...
/// This probably means that someone forgot to add `-v /path/to/Passw-pro_data/:/data`
