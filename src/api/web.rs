...
const INJECTED_SCRIPT: &str = r#"<script src="/vw_static/vault-dragdrop.js"></script>"#;
...
#[get("/css/passw-pro.css")]
fn pw_css() -> Cached<Css<String>> {
    ...
    let scss = match CONFIG.render_template("scss/passw-pro.scss", &css_options) {
        Ok(t) => t,
        Err(e) => {
            // Something went wrong loading the template. Use the fallback
            warn!("Loading scss/passw-pro.scss.hbs or scss/user.passw-pro.scss.hbs failed. {e}");
            CONFIG
                .render_fallback_template("scss/passw-pro.scss", &css_options)
                .expect("Fallback scss/passw-pro.scss.hbs to render")
        }
    };
    ...
    let css = match grass_compiler::from_string(
        ...
        Err(e) => {
            ...
            warn!("Compiling the Passw-pro SCSS styles failed. {e}");
...
async fn web_index() -> Cached<Option<Html<String>>> {
    ...
    match tokio::fs::read_to_string(&path).await {
        Ok(html) => {
            let injected = if html.contains("vault-dragdrop") {
                html
            } else {
                html.replace("</body>", &format!("{}</body>", INJECTED_SCRIPT))
            };
            Cached::short(Some(Html(injected)), false)
        }
...
#[get("/organize")]
fn organize() -> ApiResult<Html<String>> {
    let json = json!({
        "urlpath": CONFIG.domain_path()
    });
