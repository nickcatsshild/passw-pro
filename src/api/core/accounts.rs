...
pub async fn register(data: Json<RegisterData>, email_verification: bool, conn: DbConn) -> JsonResult {
    ...
    let mut user = match User::find_by_mail(&email, &conn).await {
        ...
        None => {
            ...
            // because the Passw-pro admin can invite anyone, regardless
...
async fn put_clear_device_token(device_id: DeviceId, conn: DbConn) -> EmptyResult {
    ...
    //       Passw-pro removes the push-token for all devices, but this probably means we should also unregister all these devices.
