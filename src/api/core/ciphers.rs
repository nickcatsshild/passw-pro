...
pub fn routes() -> Vec<Route> {
    ...
    //
    // Passw-pro factors in the org owner/admin status as part of
...
pub struct SearchParams {
    ...
    deleted: Option<bool>,
    #[field(name = "deletedBefore")]
    deleted_before: Option<String>,
}

#[get("/ciphers/search?<params..>")]
async fn get_ciphers_search(_params: SearchParams, headers: Headers, conn: DbConn) -> JsonResult {
    let ciphers = Cipher::find_by_user_visible(&headers.user.uuid, &conn).await;
    let cipher_sync_data = CipherSyncData::new(&headers.user.uuid, CipherSyncType::User, &conn).await;
