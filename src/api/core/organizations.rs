...
async fn put_policy(
    ...
    // Passw-pro encouraged to use multiple orgs instead of groups because groups were not available in the past
...
fn get_self_host_billing_metadata(_org_id: OrganizationId, _headers: OrgMemberHeaders) -> Json<Value> {
    // Prevent a 404 error, which also causes Javascript errors.
    Json(json!({
        "isOnSecretsManagerStandalone": false, // Secrets Manager is not supported by Passw-pro
        "organizationOccupiedSeats": 0 // Passw-pro does not count seats
    }))
}
...
async fn restore_member_vnext(
    ...
    // Passw-pro does not (yet) support the per User Collection linked to the `Enforce organization data ownership` policy.
...
// Passw-pro does not yet support exporting only managed collections!
