...
pub async fn validate_webauthn_login(user_id: &UserId, response: &str, conn: &DbConn) -> EmptyResult {
    ...
    // Passw-pro did not have knowledge of this flag prior to migrating to webauthn-rs v0.5.x
...
fn check_and_update_backup_eligible(
    ...
    if let Some(bits) = rsp.response.authenticator_data.get(32) {
        ...
        // This is needed because Passw-pro didn't store this information when using the previous version of webauthn-rs since it was a new addition to the protocol
            ...
            for reg in &mut *registrations {
                if ct_eq(reg.credential.cred_id().as_slice(), rsp_id) {
                    if reg.set_backup_eligible(backup_eligible, backup_state) {
                        ...
                        // Because Passw-pro supports multiple keys, we need to loop through the deserialized state and check which key to update
